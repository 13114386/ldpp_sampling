...
import transformers
from transformers import (AutoTokenizer, AdamW, set_seed)
...
def main():
    ...
    model = Model(args, options, logger)
    ...
    model.seq2seq.resize_token_embeddings(len(tokenizer))
    ...
    try:
        ...
        logger.warning(f"Failed to load model/optimizer checkpoint completely.")
    ...
    stop_cond = EarlyStopRougeCondition(stop_count=args.early_stop_count_on_rouge) \
                    if args.early_stop_count_on_rouge is not None and args.early_stop_count_on_rouge > 0 \
                    else None
    validate_processor = Validator(tokenizer, stop_cond)
    train_processor = Trainer(validate_processor, checkpoint)
