...
class Validator():
    ...
    def __call__(self,
        ...
        gen_kwargs = {
            "max_length": options.training.val_max_target_length,
            "num_beams": options.training.num_beams,
        }

        n_eval_iterations = min(training_config.n_eval_iterations, len(val_dataloader))
        for idx, batch in zip(range(n_eval_iterations), val_dataloader):
            with torch.no_grad():
                inputs = batch[0]
